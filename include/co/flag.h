...
inline co::vector<fastring> init(int argc, char** argv) { return flag::parse(argc, argv); }
...
namespace xx {
__coapi void add_flag(char type, const char* name, const char* value, const char* help,
...
#define _CO_DEF_FLAG(type, iden, name, value, help, ...)                                 \
    type FLG_##name = []() {                                                             \
        ::flag::xx::add_flag(iden, #name, #value, help, __FILE__, __LINE__, &FLG_##name, \
                             "" #__VA_ARGS__);                                           \
        return value;                                                                    \
    }()
...
#define DEF_uint64(name, value, help, ...) \
    _CO_DEF_FLAG(uint64_t, 'U', name, value, help, __VA_ARGS__)
