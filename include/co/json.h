...
class Array {
  public:
    ...
    static const uint32_t R = sizeof(_H) / N;
    ...
    uint32_t size() const { return _h->size; }
    bool empty() const { return this->size() == 0; }
    void resize(uint32_t n) { _h->size = n; }

    T& back() const { return _h->p[this->size() - 1]; }
    T& operator[](uint32_t i) const { return _h->p[i]; }

    void push_back(T v) {
        if (_h->size == _h->cap) {
            const size_t n = N * _h->cap;
            const size_t o = sizeof(_H) + n;
            _h = (_H*)::realloc(_h, o + n);
            assert(_h);
            _h->cap <<= 1;
        }
        _h->p[_h->size++] = v;
...
class __coapi Json {
  public:
    ...
    struct _H {
        ...
        _H(int64_t v) noexcept : type(t_int), i(v) {}
        ...
        _H(const void* p, size_t n) : type(t_string), size((uint32_t)n) {
            s = xx::alloc_string(p, n);
        }

        uint32_t type;
        uint32_t size;  // size of string
        union {
            bool b;     // for bool
            int64_t i;  // for int
            double d;   // for double
    ...
    Json(int64_t v) : _h(new(xx::alloc()) _H(v)) {}
    Json(int32_t v) : Json((int64_t)v) {}
    Json(uint32_t v) : Json((int64_t)v) {}
    Json(uint64_t v) : Json((int64_t)v) {}
    ...
    int32_t as_int32() const { return (int32_t)this->as_int64(); }
    ...
    Json& get(uint32_t i) const;
    Json& get(int i) const { return this->get((uint32_t)i); }
    ...
    void remove(int i) { this->remove((uint32_t)i); }
    ...
    void erase(int i) { this->erase((uint32_t)i); }
    void erase(const char* key);

    // it is better to use get() instead of this method.
    Json& operator[](uint32_t i) const {
        assert(this->is_array() && !_array().empty());
        return (Json&)_array()[i];
    }

    Json& operator[](int i) const { return this->operator[]((uint32_t)i); }

    bool operator==(bool v) const { return this->is_bool() && _h->b == v; }
    bool operator==(double v) const { return this->is_double() && _h->d == v; }
    bool operator==(int64_t v) const { return this->is_int() && _h->i == v; }
    bool operator==(int v) const { return this->operator==((int64_t)v); }
    bool operator==(uint32_t v) const { return this->operator==((int64_t)v); }
    bool operator==(uint64_t v) const { return this->operator==((int64_t)v); }
    ...
    bool operator!=(int64_t v) const { return !this->operator==(v); }
    bool operator!=(int v) const { return !this->operator==(v); }
    bool operator!=(uint32_t v) const { return !this->operator==(v); }
    bool operator!=(uint64_t v) const { return !this->operator==(v); }
    ...
    uint32_t string_size() const { return (_h && (_h->type & t_string)) ? _h->size : 0; }
    ...
    class iterator {
      public:
        typedef void* T;
        iterator(T* p, T* e, uint32_t step) : _p(p), _e(e), _step(step) {}

        struct End {
            constexpr End() noexcept {}
        };  // fake end
        static const End& end() {
            static const End kEnd;
            return kEnd;
        }
    ...
    fastring& pretty(fastring& s, int mdp = 16) const {
        return (fastring&)this->pretty((fastream&)s, mdp);
    }
  ...
  private:
    ...
    Json& _set(uint32_t i);
    Json& _set(int i) { return this->_set((uint32_t)i); }
