...
#ifdef _WIN32
    if (color::ansi_esc_seq_enabled()) return os << get_color_str(c);
    os.flush();
    auto h = color::cout_handle();
    if (h) SetConsoleTextAttribute(h, (WORD)get_win_fgi(c));
    return os;
#else
    return os << get_color_str(c);
#endif
}
...
struct Printer {
    inline Printer() : s(cstream()) { n = s.size(); }
    inline ~Printer() {
        s << '\n';
        {
