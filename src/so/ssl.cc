...
const char* strerror(S* s) {
    static thread_local fastream _fs(256);

    _fs.clear();

    if (ERR_peek_error() != 0) {
        ERR_print_errors_cb(errcb, &_fs);
    } else if (co::error() != 0) {
        _fs.append(co::strerror());
    } else if (s) {
...
void free_ctx(C* c) { SSL_CTX_free((SSL_CTX*)c); }

void* new_ssl(C* c) { return (void*)SSL_new((SSL_CTX*)c); }

void free_ssl(S* s) { SSL_free((SSL*)s); }

int set_fd(S* s, int fd) { return SSL_set_fd((SSL*)s, fd); }

int get_fd(const S* s) { return SSL_get_fd((const SSL*)s); }
...
int check_private_key(const C* c) { return SSL_CTX_check_private_key((const SSL_CTX*)c); }
...
int shutdown(S* s, int ms) {
    ...
    do {
        ...
        if (r == 0) {
            TLOG << "SSL_shutdown return 0, call again..";
            continue;
        }
        ...
        if (e == SSL_ERROR_WANT_READ) {
            ...
            TLOG << "SSL_shutdown return " << r << ", error: " << e;
            return r;
        }
    } while (true);
}
...
int accept(S* s, int ms) {
    ...
    do {
        ...
        if (r == 1) return 1;  // success
        if (r == 0) {
            // TLOG << "SSL_accept return 0, error: " << SSL_get_error(s, 0);
        ...
        if (e == SSL_ERROR_WANT_READ) {
            ...
            // TLOG << "SSL_accept return " << r << ", error: " << e;
            return r;
        }
    } while (true);
}
...
int connect(S* s, int ms) {
    ...
    do {
        ...
        if (r == 1) return 1;  // success
        if (r == 0) {
            // TLOG << "SSL_connect return 0, error: " << SSL_get_error(s, 0);
            return 0;  // ssl connection shut down
        }
        ...
        if (e == SSL_ERROR_WANT_READ) {
            ...
            // TLOG << "SSL_connect return " << r << ", error: " << e;
            return r;
        }
    } while (true);
}
...
int recv(S* s, void* buf, int n, int ms) {
    ...
    do {
        ...
        if (r == 0) {
            // TLOG << "SSL_read return 0, error: " << SSL_get_error(s, 0);
            return 0;
        }
        ...
        if (e == SSL_ERROR_WANT_READ) {
            ...
            // TLOG << "SSL_read return " << r << ", error: " << e;
            return r;
        }
    } while (true);
}
...
int send(S* s, const void* buf, int n, int ms) {
    ...
    do {
        ...
        if (r == remain) return n;  // success
        if (r == 0) {
            // TLOG << "SSL_write return 0, error: " << SSL_get_error(s, 0);
            return 0;
        }
        ...
        if (r < 0) {
            ...
            if (e == SSL_ERROR_WANT_READ) {
                ...
                // TLOG << "SSL_write return " << r << ", error: " << e;
                return r;
            }
        } else {
            remain -= r;
...
C* new_ctx(char) {
    CHECK(false) << "To use SSL features, please build libco with openssl 1.1.0+ as follow: \n"
