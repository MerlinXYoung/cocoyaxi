...
struct ref_counter
{
    constexpr inline ref_counter() noexcept
        : _refn(1) {}
    constexpr inline ref_counter(uint32_t n) noexcept
        : _refn(n) {}
    ~ref_counter() = default;
...
bool event_impl::wait(uint32_t ms) {
    ...
    if (sched) {                              /* in coroutine */
        ...
        ++_wt;
        if (ms != (uint32_t)-1) {
            const auto r = _cv.wait_for(g, std::chrono::milliseconds(ms)) == std::cv_status::no_timeout;
            if (!r && sn == _sn) {
                assert(_wt > 0);
                --_wt;
            }
        ...
        if (!_wc.empty()) {
            ...
            if (!has_wt) {
                do {
                    ...
                    if (w->state.compare_exchange_strong(state, st_ready, std::memory_order_relaxed, std::memory_order_relaxed)) {
                        has_wc = true;
...
public:
    explicit inline pipe_impl(uint32_t buf_size, uint32_t blk_size, uint32_t ms, pipe::C&& c, pipe::D&& d)
        : ref_counter()
...
void pipe_impl::read(void* p) {
    ...
    if (_full) {
        if (_buf) {
            ...
            while (!_wq.empty()) {
                ...
                decltype(w->state)::value_type state(st_wait);
                if (_ms == (uint32_t)-1 ||
                    w->state.compare_exchange_strong(state, st_ready, std::memory_order_relaxed, std::memory_order_relaxed)) {
    ...
    if (sched) {
        ...
        for (;;) {
            ...
            decltype(w->state)::value_type state(st_wait);
            if (r || !w->state.compare_exchange_strong(state, st_timeout, std::memory_order_relaxed, std::memory_order_relaxed)) {
...
void pipe_impl::close() {
    ...
    _closed.compare_exchange_strong(closed, 1, std::memory_order_relaxed, std::memory_order_relaxed);
    if (closed == 0) {
        std::unique_lock<std::mutex> g(_m);
...
void pipe::read(void* p) const {
    reinterpret_cast<pipe_impl*>(_p)->read(p);
}

void pipe::write(void* p, int v) const {
    reinterpret_cast<pipe_impl*>(_p)->write(p, v);
}

bool pipe::done() const noexcept {
    return reinterpret_cast<pipe_impl*>(_p)->done();
}

void pipe::close() const {
    reinterpret_cast<pipe_impl*>(_p)->close();
}

bool pipe::is_closed() const noexcept {
    return reinterpret_cast<pipe_impl*>(_p)->is_closed();
}
...
public:
    explicit inline pipe_impl_cap(uint32_t cap, uint32_t blk_size, uint32_t ms, pipe::C&& c, pipe::D&& d)
        : ref_counter()
        , _cap(cap)
        , _size(0)
        , _blk_size(blk_size)
    ...
    struct waitx : co::clink
    {
        explicit inline waitx(Coroutine* _co, void* _buf)
            : co(_co)
            , state(st_wait)
            , buf(_buf) {
            x.done = 0;
...
inline void pipe_impl_cap::_write_block(void* dst, void* p, int v) {
    _c(dst, p, v);
}
...
pipe_cap::pipe_cap(const pipe_cap& p)
    : _p(p._p) {
...
void pipe_cap::read(void* p) const {
    reinterpret_cast<pipe_impl_cap*>(_p)->read(p);
}

void pipe_cap::write(void* p, int v) const {
    reinterpret_cast<pipe_impl_cap*>(_p)->write(p, v);
}

bool pipe_cap::done() const noexcept {
    return reinterpret_cast<pipe_impl_cap*>(_p)->done();
}

void pipe_cap::close() const {
    reinterpret_cast<pipe_impl_cap*>(_p)->close();
}
...
void mutex::lock() const {
    reinterpret_cast<xx::mutex_impl*>(_p)->lock();
}

void mutex::unlock() const {
    reinterpret_cast<xx::mutex_impl*>(_p)->unlock();
}

bool mutex::try_lock() const noexcept {
    return reinterpret_cast<xx::mutex_impl*>(_p)->try_lock();
}

event::event(bool manual_reset, bool signaled)
    : _p(new xx::event_impl(manual_reset, signaled)) {}
...
bool event::wait(uint32_t ms) const {
    return reinterpret_cast<xx::event_impl*>(_p)->wait(ms);
}

void event::signal() const {
    reinterpret_cast<xx::event_impl*>(_p)->signal();
}

void event::reset() const {
    reinterpret_cast<xx::event_impl*>(_p)->reset();
}
...
void sync_event::signal() {
    reinterpret_cast<xx::sync_event_impl*>(_p)->signal();
}

void sync_event::reset() {
    reinterpret_cast<xx::sync_event_impl*>(_p)->reset();
}

void sync_event::wait() {
    reinterpret_cast<xx::sync_event_impl*>(_p)->wait();
}

bool sync_event::wait(uint32_t ms) {
    return reinterpret_cast<xx::sync_event_impl*>(_p)->wait(ms);
}

wait_group::wait_group(uint32_t n)
    : _p(new xx::event_impl(false, false, n)) {}

wait_group::wait_group(const wait_group& wg)
    : _p(wg._p) {
    if (_p) reinterpret_cast<xx::event_impl*>(_p)->ref();
}
...
void wait_group::wait() const {
    reinterpret_cast<xx::event_impl*>(_p)->wait((uint32_t)-1);
}
...
void* pool::pop() const {
    return reinterpret_cast<xx::pool_impl*>(_p)->pop();
}

void pool::push(void* p) const {
    reinterpret_cast<xx::pool_impl*>(_p)->push(p);
}

void pool::clear() const {
    reinterpret_cast<xx::pool_impl*>(_p)->clear();
}

size_t pool::size() const noexcept {
    return reinterpret_cast<xx::pool_impl*>(_p)->size();
}
