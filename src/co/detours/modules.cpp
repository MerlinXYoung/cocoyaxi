...
const GUID DETOUR_EXE_RESTORE_GUID = {
    0xbda26f34, 0xbc82, 0x4829, {0x9e, 0x64, 0x74, 0x2c, 0x4, 0xc8, 0x4f, 0xa0}};
...
PDETOUR_SYM_INFO DetourLoadImageHlp(VOID) {
    ...
    symInfo.pfImagehlpApiVersionEx =
        (PF_ImagehlpApiVersionEx)GetProcAddress(symInfo.hDbgHelp, "ImagehlpApiVersionEx");
    ...
    if (symInfo.pfImagehlpApiVersionEx == nullptr || symInfo.pfSymInitialize == nullptr ||
        symInfo.pfSymLoadModule64 == nullptr || symInfo.pfSymGetModuleInfo64 == nullptr ||
        symInfo.pfSymFromName == nullptr) {
        goto abort;
    }
...
#pragma prefast(suppress : 28752, "We don't do the unicode conversion for LoadLibraryExA.")
    ...
    if (pSymInfo->pfSymLoadModule64(pSymInfo->hProcess, nullptr, (PCHAR)pszModule, nullptr,
                                    (DWORD64)hModule, 0) == 0) {
...
#pragma warning(suppress : 6011)  // GetModuleHandleW(nullptr) never returns nullptr.
        ...
        if (pNtHeader->OptionalHeader.Magic == IMAGE_NT_OPTIONAL_HDR32_MAGIC) {
            if (((PIMAGE_NT_HEADERS32)pNtHeader)->CLR_DIRECTORY.VirtualAddress != 0 &&
                ...
                pClrHeader =
                    (PDETOUR_CLR_HEADER)(((PBYTE)pDosHeader) + ((PIMAGE_NT_HEADERS32)pNtHeader)
                                                                   ->CLR_DIRECTORY.VirtualAddress);
            }
            ...
            if (((PIMAGE_NT_HEADERS64)pNtHeader)->CLR_DIRECTORY.VirtualAddress != 0 &&
                ...
                pClrHeader =
                    (PDETOUR_CLR_HEADER)(((PBYTE)pDosHeader) + ((PIMAGE_NT_HEADERS64)pNtHeader)
                                                                   ->CLR_DIRECTORY.VirtualAddress);
            }
        }
        ...
        // so handle them by returning nullptr (LastError is NO_ERROR)
...
#pragma warning(suppress : 6011)  // GetModuleHandleW(nullptr) never returns nullptr.
        ...
        for (DWORD nFunc = 0; nFunc < pExportDir->NumberOfFunctions; nFunc++) {
            PBYTE pbCode = (pdwFunctions != nullptr)
                               ? (PBYTE)RvaAdjust(pDosHeader, pdwFunctions[nFunc])
                               : nullptr;
            ...
            for (DWORD n = 0; n < pExportDir->NumberOfNames; n++) {
                if (pwOrdinals[n] == nFunc) {
                    pszName =
                        (pdwNames != nullptr) ? (PCHAR)RvaAdjust(pDosHeader, pdwNames[n]) : nullptr;
                    break;
                }
            }
...
#pragma warning(suppress : 6011)  // GetModuleHandleW(nullptr) never returns nullptr.
        ...
        for (; iidp->OriginalFirstThunk != 0; iidp++) {
            ...
            PIMAGE_THUNK_DATA pThunks =
                (PIMAGE_THUNK_DATA)RvaAdjust(pDosHeader, iidp->OriginalFirstThunk);
...
static BOOL CALLBACK DetourEnumerateImportsThunk(_In_ PVOID VoidContext, _In_ DWORD nOrdinal,
                                                 _In_opt_ PCSTR pszFunc, _In_opt_ PVOID *ppvFunc) {
    _DETOUR_ENUMERATE_IMPORTS_THUNK_CONTEXT const *const pContext =
        (_DETOUR_ENUMERATE_IMPORTS_THUNK_CONTEXT *)VoidContext;
    return pContext->pfImportFunc(pContext->pContext, nOrdinal, pszFunc,
...
BOOL WINAPI DetourEnumerateImports(_In_opt_ HMODULE hModule, _In_opt_ PVOID pContext,
    ...
    return DetourEnumerateImportsEx(hModule, (PVOID)&context, pfImportFile,
...
#pragma warning(suppress : 6011)  // GetModuleHandleW(nullptr) never returns nullptr.
        ...
        PIMAGE_SECTION_HEADER pSectionHeaders =
            (PIMAGE_SECTION_HEADER)((PBYTE)pNtHeader + sizeof(pNtHeader->Signature) +
...
BOOL WINAPI DetourFreePayload(_In_ PVOID pvData) {
    ...
    // If you have any doubts about the following code, please refer to the comments in
    // DetourCopyPayloadToProcess.
    HMODULE hModule = DetourGetContainingModule(pvData);
    DETOUR_ASSERT(hModule != nullptr);
    if (hModule != nullptr) {
...
BOOL WINAPI DetourRestoreAfterWithEx(_In_reads_bytes_(cbData) PVOID pvData, _In_ DWORD cbData) {
    ...
    if (DetourVirtualProtectSameExecute(pder->pidh, pder->cbidh, PAGE_EXECUTE_READWRITE,
                                        &dwPermIdh)) {
        if (DetourVirtualProtectSameExecute(pder->pinh, pder->cbinh, PAGE_EXECUTE_READWRITE,
                                            &dwPermInh)) {
            CopyMemory(pder->pidh, &pder->idh, pder->cbidh);
            CopyMemory(pder->pinh, &pder->inh, pder->cbinh);

            if (pder->pclr != nullptr && !fUpdated32To64) {
                if (DetourVirtualProtectSameExecute(pder->pclr, pder->cbclr, PAGE_EXECUTE_READWRITE,
                                                    &dwPermClr)) {
