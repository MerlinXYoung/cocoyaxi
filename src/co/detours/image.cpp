...
_Must_inspect_result_ static inline HRESULT StringCchLengthA(
    _In_reads_or_z_(cchMax) LPCSTR psz, _In_ _In_range_(1, STRSAFE_MAX_CCH) size_t cchMax,
...
_Must_inspect_result_ static inline HRESULT StringCchCopyA(_Out_writes_(cchDest) _Always_(_Post_z_)
                                                               LPSTR pszDest,
                                                           _In_ size_t cchDest,
                                                           _In_ LPCSTR pszSrc) {
    HRESULT hr = S_OK;
...
_Must_inspect_result_ static inline HRESULT StringCchCatA(_Out_writes_(cchDest) _Always_(_Post_z_)
                                                              LPSTR pszDest,
...
PBYTE CImageData::Enumerate(GUID *pGuid, DWORD *pcbData, DWORD *pnIterator) {
    ...
    if (pnIterator == nullptr || m_cbData < *pnIterator + sizeof(DETOUR_SECTION_RECORD)) {
        if (pcbData) {
            *pcbData = 0;
        }
        if (pGuid) {
...
class CImageThunks {
  ...
  public:
    CImageThunks(CImage *pImage, DWORD nThunksMax, DWORD *pnAddr) {
        ...
        m_pThunks = (PIMAGE_THUNK_DATA)m_pImage->AllocateOutput(
            sizeof(IMAGE_THUNK_DATA) * nThunksMax, &m_nThunkVirtAddr);
...
PVOID CImage::RvaToVa(ULONG_PTR nRva) {
    ...
    for (DWORD n = 0; n < m_NtHeader.FileHeader.NumberOfSections; n++) {
        ...
        if (nRva >= vaStart && nRva < vaEnd) {
            return (PBYTE)m_pMap + m_SectionHeaders[n].PointerToRawData + nRva -
...
BOOL CImage::Read(HANDLE hFile) {
    ...
    for (n = 0; n < m_NtHeader.FileHeader.NumberOfSections; n++) {
        m_nExtraOffset =
            Max(m_SectionHeaders[n].PointerToRawData + m_SectionHeaders[n].SizeOfRawData,
                m_nExtraOffset);
        ...
        if (strcmp((PCHAR)m_SectionHeaders[n].Name, ".detour") == 0) {
            ...
            m_NtHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT]
                .VirtualAddress = dh.nOriginalBoundImportVirtualAddress;
            ...
            m_NtHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IAT].Size =
                dh.nOriginalIatSize;
...
BOOL CImage::CheckImportsNeeded(DWORD *pnTables, DWORD *pnThunks, DWORD *pnChars) {
    ...
    for (CImageImportFile *pImportFile = m_pImportFiles; pImportFile != nullptr;
    ...
    while ((pImportFile = *ppLastFile) != nullptr) {
        ...
        if (pImportFile->m_fByway) {
            ...
            if (pfFileCallback != nullptr) {
                ...
                if (!(*pfFileCallback)(pContext, pImportFile->m_pszOrig, pImportFile->m_pszName,
                                       &pszFile)) {
                    goto fail;
                }
            ...
            if (pfSymbolCallback != nullptr) {
                for (DWORD n = 0; n < pImportFile->m_nImportNames; n++) {
                    ...
                    ULONG nOrdinal = 0;
                    if (!(*pfSymbolCallback)(
                            pContext, pImportName->m_nOrig, pImportName->m_nOrdinal, &nOrdinal,
...
BOOL CImage::Write(HANDLE hFile) {
    ...
    if (fNeedDetourSection || !m_pImageData->IsEmpty()) {
        ...
        m_nSectionsOffset = m_nPeOffset + sizeof(m_NtHeader.Signature) +
    ...
    for (; n < m_NtHeader.FileHeader.NumberOfSections; n++) {
        if (m_SectionHeaders[n].SizeOfRawData) {
            if (SetFilePointer(hFile, m_SectionHeaders[n].PointerToRawData, nullptr, FILE_BEGIN) ==
                ~0u) {
                return FALSE;
            }
            if (!CopyFileData(hFile, m_SectionHeaders[n].PointerToRawData,
    ...
    if (fNeedDetourSection || !m_pImageData->IsEmpty()) {
        ...
        DWORD clrAddr =
            m_NtHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR]
                .VirtualAddress;
        DWORD clrSize =
            m_NtHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR].Size;
        if (clrAddr && clrSize) {
        ...
        HRESULT hrRet = StringCchCopyA((PCHAR)m_SectionHeaders[nSection].Name,
                                       IMAGE_SIZEOF_SHORT_NAME, ".detour");
        ...
        if (!SizeOutputBuffer(QuadAlign(sizeof(dh)) + m_cbPrePE +
        ...
        //////////////////////////////////////////////////////////////////////////
        //
        if (SetFilePointer(hFile, m_SectionHeaders[nSection].PointerToRawData, nullptr,
                           FILE_BEGIN) == ~0u) {
            return FALSE;
        }
    ...
    //
    DWORD debugAddr =
        m_NtHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_DEBUG].VirtualAddress;
    ...
    /////////////////////////////////////////////////////// Adjust CLR Header.
    //
    DWORD clrAddr = m_NtHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR]
                        .VirtualAddress;
    DWORD clrSize =
        m_NtHeader.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR].Size;
    ...
    if (!WriteFile(hFile, &m_SectionHeaders,
                   sizeof(m_SectionHeaders[0]) * m_NtHeader.FileHeader.NumberOfSections, &cbDone)) {
        return FALSE;
    }
...
static BOOL CALLBACK ResetFileCallback(_In_opt_ PVOID pContext, _In_ LPCSTR pszOrigFile,
                                       _In_ LPCSTR pszFile,
                                       _Outptr_result_maybenull_ LPCSTR *ppszOutFile) {
    UNREFERENCED_PARAMETER(pContext);
    UNREFERENCED_PARAMETER(pszFile);
...
static BOOL CALLBACK ResetSymbolCallback(_In_opt_ PVOID pContext, _In_ ULONG nOrigOrdinal,
...
BOOL WINAPI DetourBinaryResetImports(_In_ PDETOUR_BINARY pBinary) {
    ...
    return pImage->EditImports(nullptr, ResetBywayCallback, ResetFileCallback, ResetSymbolCallback,
                               nullptr);
}
