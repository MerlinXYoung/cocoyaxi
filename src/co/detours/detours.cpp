...
int Detour_AssertExprWithFunctionName(int reportType, const char *filename, int linenumber,
                                      const char *FunctionName, const char *msg) {
    int nRet = 0;
    DWORD dwLastError = GetLastError();
    CHAR szModuleNameWithFunctionName[MAX_PATH * 2];
    szModuleNameWithFunctionName[0] = 0;
    GetModuleFileNameA((HMODULE)&__ImageBase, szModuleNameWithFunctionName,
                       ARRAYSIZE(szModuleNameWithFunctionName));
    StringCchCatNA(
        szModuleNameWithFunctionName, ARRAYSIZE(szModuleNameWithFunctionName), ",",
        ARRAYSIZE(szModuleNameWithFunctionName) - strlen(szModuleNameWithFunctionName) - 1);
    StringCchCatNA(
        szModuleNameWithFunctionName, ARRAYSIZE(szModuleNameWithFunctionName), FunctionName,
        ARRAYSIZE(szModuleNameWithFunctionName) - strlen(szModuleNameWithFunctionName) - 1);
...
inline ULONG_PTR detour_2gb_above(ULONG_PTR address) {
    ...
    return (address < (ULONG_PTR)0xffffffff80000000) ? address + 0x7ff80000
                                                     : (ULONG_PTR)0xfffffffffff80000;
#else
...
inline void detour_find_jmp_bounds(PBYTE pbCode, PDETOUR_TRAMPOLINE *ppLower,
...
inline ULONG detour_is_code_filler(PBYTE pbCode) {
    ...
    if (pbCode[0] == 0x0F && pbCode[1] == 0x1F && pbCode[2] == 0x44 && pbCode[3] == 0x00 &&
        pbCode[4] == 0x00) {
        return 5;
    }
    if (pbCode[0] == 0x66 && pbCode[1] == 0x0F && pbCode[2] == 0x1F && pbCode[3] == 0x44 &&
        pbCode[4] == 0x00 && pbCode[5] == 0x00) {
        return 6;
    }
    if (pbCode[0] == 0x0F && pbCode[1] == 0x1F && pbCode[2] == 0x80 && pbCode[3] == 0x00 &&
        pbCode[4] == 0x00 && pbCode[5] == 0x00 && pbCode[6] == 0x00) {
        return 7;
    }
    if (pbCode[0] == 0x0F && pbCode[1] == 0x1F && pbCode[2] == 0x84 && pbCode[3] == 0x00 &&
        pbCode[4] == 0x00 && pbCode[5] == 0x00 && pbCode[6] == 0x00 && pbCode[7] == 0x00) {
        return 8;
    }
    if (pbCode[0] == 0x66 && pbCode[1] == 0x0F && pbCode[2] == 0x1F && pbCode[3] == 0x84 &&
        pbCode[4] == 0x00 && pbCode[5] == 0x00 && pbCode[6] == 0x00 && pbCode[7] == 0x00 &&
        pbCode[8] == 0x00) {
        return 9;
    }
    if (pbCode[0] == 0x66 && pbCode[1] == 0x66 && pbCode[2] == 0x0F && pbCode[3] == 0x1F &&
        pbCode[4] == 0x84 && pbCode[5] == 0x00 && pbCode[6] == 0x00 && pbCode[7] == 0x00 &&
        pbCode[8] == 0x00 && pbCode[9] == 0x00) {
        return 10;
    }
    if (pbCode[0] == 0x66 && pbCode[1] == 0x66 && pbCode[2] == 0x66 && pbCode[3] == 0x0F &&
        pbCode[4] == 0x1F && pbCode[5] == 0x84 && pbCode[6] == 0x00 && pbCode[7] == 0x00 &&
        pbCode[8] == 0x00 && pbCode[9] == 0x00 && pbCode[10] == 0x00) {
        return 11;
    }
...
inline PBYTE detour_skip_jmp(PBYTE pPointer, PVOID *ppGlobals) {
    ...
    if ((pb[0].wide[0] & 0xfffffc000603ffff) == 0x002024000200100b &&
        pb[0].wide[1] == 0x0004000000203008 && pb[1].wide[0] == 0x001014180420180a &&
        pb[1].wide[1] == 0x07000830c0203008 && pb[2].wide[0] == 0x0000000100000010 &&
    ...
    if ((Opcode & 0xfbf08f00) == 0xf2400c00) {  // movw r12,#xxxx
        ...
        if ((Opcode2 & 0xfbf08f00) == 0xf2c00c00) {  // movt r12,#xxxx
            ...
            if (Opcode3 == 0xf8dcf000) {  // ldr  pc,[r12]
                PBYTE pbTarget =
                    (PBYTE)(((Opcode2 << 12) & 0xf7000000) | ((Opcode2 << 1) & 0x08000000) |
...
struct _DETOUR_TRAMPOLINE {
    ...
    //   br or brl
    //
    // A theoretical maximum for rbCode is thefore 4*4*6 + 16 = 112 (another 16 for jmp to
    // pbRemain).
    //
...
PBYTE detour_gen_jmp_indirect(BYTE *pbCode, ULONG64 *pbJmpVal) {
    ...
    jmpIndAddr.value =
        (((LONG64)pbJmpVal) & 0xFFFFFFFFFFFFF000) - (((LONG64)pbCode) & 0xFFFFFFFFFFFFF000);
        ...
        if ((Opcode2 & 0xffe003ff) == 0xf9400210) {  // ldr   x16, [x16, IAT]
            ...
            if (Opcode3 == 0xd61f0200) {  // br    x16
                ...
                /* https://static.docs.arm.com/ddi0487/bb/DDI0487B_b_armv8_arm.pdf
                    The ADRP instruction shifts a signed, 21-bit immediate left by 12 bits, adds it
                to the value of the program counter with the bottom 12 bits cleared to zero, and
                then writes the result to a general-purpose register. This permits the calculation
                of the address at a 4KB aligned memory region. In conjunction with an ADD
                (immediate) instruction, or a Load/Store instruction with a 12-bit immediate offset,
                this allows for the calculation of, or access to, any address within +/- 4GB of the
                current PC.

                PC-rel. addressing
                    This section describes the encoding of the PC-rel. addressing instruction class.
                The encodings in this section are decoded from Data Processing -- Immediate on page
                C4-226. Add/subtract (immediate) This section describes the encoding of the
                Add/subtract (immediate) instruction class. The encodings in this section are
                decoded from Data Processing -- Immediate on page C4-226. Decode fields Instruction
                page op 0 ADR 1 ADRP

                C6.2.10 ADRP
                    Form PC-relative address to 4KB page adds an immediate value that is shifted
                left by 12 bits, to the PC value to form a PC-relative address, with the bottom 12
                bits masked out, and writes the result to the destination register. ADRP <Xd>,
                <label> imm = SignExtend(immhi:immlo:Zeros(12), 64);
                ...
                */
                ...
                register and an immediate offset. The Unsigned offset variant scales the immediate
                offset value by the size of the value accessed before adding it to the base register
                value.
                ...
                */
                ...
                PBYTE const pbTarget =
                    (PBYTE)((ULONG64)pbCode & 0xfffffffffffff000ULL) + page + offset;
...
static PVOID detour_alloc_region_from_lo(PBYTE pbLo, PBYTE pbHi) {
    ...
    for (; pbTry < pbHi;) {
        ...
        DETOUR_TRACE(("  Try %p => %p..%p %6lx\n", pbTry, mbi.BaseAddress,
                      (PBYTE)mbi.BaseAddress + mbi.RegionSize - 1, mbi.State));

        if (mbi.State == MEM_FREE && mbi.RegionSize >= DETOUR_REGION_SIZE) {
            PVOID pv = VirtualAlloc(pbTry, DETOUR_REGION_SIZE, MEM_COMMIT | MEM_RESERVE,
...
static PVOID detour_alloc_region_from_hi(PBYTE pbLo, PBYTE pbHi) {
    ...
    for (; pbTry > pbLo;) {
        ...
        if (mbi.State == MEM_FREE && mbi.RegionSize >= DETOUR_REGION_SIZE) {
            ...
            pbTry =
                detour_alloc_round_down_to_region((PBYTE)mbi.AllocationBase - DETOUR_REGION_SIZE);
        }
    }
    return nullptr;
}

static PVOID detour_alloc_trampoline_allocate_new(PBYTE pbTarget, PDETOUR_TRAMPOLINE pLo,
...
static PDETOUR_TRAMPOLINE detour_alloc_trampoline(PBYTE pbTarget) {
    ...
    if (s_pRegion != nullptr && s_pRegion->pFree != nullptr && s_pRegion->pFree >= pLo &&
        s_pRegion->pFree <= pHi) {
    found_region:
    ...
    if (pbNewlyAllocated != nullptr) {
        ...
        DETOUR_TRACE(("  Allocated region %p..%p\n\n", s_pRegion,
                      ((PBYTE)s_pRegion) + DETOUR_REGION_SIZE - 1));
...
LONG WINAPI DetourTransactionCommit() { return DetourTransactionCommitEx(nullptr); }
        ...
        if (o->fIsRemove) {
            ...
            *o->ppbPointer = o->pbTarget;
            ...
            DETOUR_TRACE(("detours: pbTramp =%p, pbRemain=%p, pbDetour=%p, cbRestore=%u\n",
                          o->pTrampoline, o->pTrampoline->pbRemain, o->pTrampoline->pbDetour,
            ...
            DETOUR_TRACE(
                ("detours: pbTarget=%p: "
                 ...
                 o->pbTarget, o->pbTarget[0], o->pbTarget[1], o->pbTarget[2], o->pbTarget[3],
                 o->pbTarget[4], o->pbTarget[5], o->pbTarget[6], o->pbTarget[7], o->pbTarget[8],
                 o->pbTarget[9], o->pbTarget[10], o->pbTarget[11]));
...
#ifdef DETOURS_ARM64
            PBYTE pbCode =
                detour_gen_jmp_indirect(o->pbTarget, (ULONG64 *)&(o->pTrampoline->pbDetour));
            pbCode = detour_gen_brk(pbCode, o->pTrampoline->pbRemain);
            *o->ppbPointer = o->pTrampoline->rbCode;
            UNREFERENCED_PARAMETER(pbCode);
#endif  // DETOURS_ARM64
            ...
            DETOUR_TRACE(
                ("detours: pbTramp =%p: "
                 ...
                 "%02x %02x %02x %02x "
                 "%02x %02x %02x %02x\n",
                 o->pTrampoline, o->pTrampoline->rbCode[0], o->pTrampoline->rbCode[1],
                 o->pTrampoline->rbCode[2], o->pTrampoline->rbCode[3], o->pTrampoline->rbCode[4],
                 o->pTrampoline->rbCode[5], o->pTrampoline->rbCode[6], o->pTrampoline->rbCode[7],
                 o->pTrampoline->rbCode[8], o->pTrampoline->rbCode[9], o->pTrampoline->rbCode[10],
                 o->pTrampoline->rbCode[11]));
            ...
            for (o = s_pPendingOperations; o != nullptr; o = o->pNext) {
                if (o->fIsRemove) {
                    if (cxt.DETOURS_EIP >= (DETOURS_EIP_TYPE)(ULONG_PTR)o->pTrampoline &&
                        ...
                        cxt.DETOURS_EIP =
                            (DETOURS_EIP_TYPE)((ULONG_PTR)o->pbTarget +
...
LONG WINAPI DetourAttachEx(_Inout_ PVOID *ppPointer, _In_ PVOID pDetour,
    ...
    while (cbTarget < cbJump) {
        ...
        pbSrc =
            (PBYTE)DetourCopyInstruction(pbTrampoline, (PVOID *)&pbPool, pbSrc, nullptr, &lExtra);
...
#if DETOUR_DEBUG
    {
        ...
        for (n = 0; n < ARRAYSIZE(pTrampoline->rAlign); n++) {
            ...
            DETOUR_TRACE(
                (" %u/%u", pTrampoline->rAlign[n].obTarget, pTrampoline->rAlign[n].obTrampoline));
        }
        DETOUR_TRACE((" ]\n"));
    }
#endif
    ...
    DETOUR_TRACE(
        ("detours: pbTarget=%p: "
         ...
         "%02x %02x %02x %02x "
         "%02x %02x %02x %02x\n",
         pbTarget, pbTarget[0], pbTarget[1], pbTarget[2], pbTarget[3], pbTarget[4], pbTarget[5],
         pbTarget[6], pbTarget[7], pbTarget[8], pbTarget[9], pbTarget[10], pbTarget[11]));
    DETOUR_TRACE(
        ("detours: pbTramp =%p: "
         "%02x %02x %02x %02x "
         "%02x %02x %02x %02x "
         "%02x %02x %02x %02x\n",
         pTrampoline, pTrampoline->rbCode[0], pTrampoline->rbCode[1], pTrampoline->rbCode[2],
         pTrampoline->rbCode[3], pTrampoline->rbCode[4], pTrampoline->rbCode[5],
         pTrampoline->rbCode[6], pTrampoline->rbCode[7], pTrampoline->rbCode[8],
         pTrampoline->rbCode[9], pTrampoline->rbCode[10], pTrampoline->rbCode[11]));
...
LONG WINAPI DetourDetach(_Inout_ PVOID *ppPointer, _In_ PVOID pDetour) {
    ...
    pDetour = (PBYTE)DetourCodeFromPointer(ppldDetour, &pDetourGlobals);
    PDETOUR_TRAMPOLINE pTrampoline =
        (PDETOUR_TRAMPOLINE)DetourCodeFromPointer(ppldTrampo, &pTrampoGlobals);
    DETOUR_TRACE(("  ppldDetour=%p, code=%p [gp=%p]\n", ppldDetour, pDetour, pDetourGlobals));
    ...
    DETOUR_TRACE(("\n"));
#else   // !DETOURS_IA64
    PDETOUR_TRAMPOLINE pTrampoline = (PDETOUR_TRAMPOLINE)DetourCodeFromPointer(*ppPointer, nullptr);
    pDetour = DetourCodeFromPointer(pDetour, nullptr);
#endif  // !DETOURS_IA64
...
_Success_(return != FALSE) BOOL WINAPI
    ...
    return DetourVirtualProtectSameExecuteEx(GetCurrentProcess(), pAddress, nSize, dwNewProtect,
                                             pdwOldProtect);
}
