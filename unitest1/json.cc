...
TEST_CASE("initializer_list", "[json]") {
    co::Json a = {1, 2, 3};
    EXPECT(a.is_array());
    EXPECT_EQ(a.size(), 3);
...
TEST_CASE("parse_null", "[json]") {
    co::Json v;
    EXPECT(v.parse_from("null"));
    EXPECT(v.is_null());
}

TEST_CASE("parse_bool", "[json]") {
