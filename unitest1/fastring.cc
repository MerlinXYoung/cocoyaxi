...
TEST_CASE("constructor", "[fastring]") {
    {
        fastring s;
        EXPECT(s.empty());
        EXPECT_EQ(s.size(), 0);
...
TEST_CASE("starts_ends", "[fastring]") {
    fastring s("xxxyyyzzz");
...
TEST_CASE("remove_suffix", "[fastring]") {
    fastring s("xx.log");
...
TEST_CASE("remove_prefix", "[fastring]") {
    fastring s("xxhello");
